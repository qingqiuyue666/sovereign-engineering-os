...
from kernel.lifecycle import (
    runtime_checker_implementation_boundary_validator as module,
)
from kernel.lifecycle.runtime_checker_implementation_boundary_validator import (
...
def valid_boundary():
    manifest = runtime_checker_implementation_boundary_validator_manifest()
    return {
        "surface": "RuntimeCheckerImplementationBoundaryV1",
        "version": 1,
        "source_refs": copy.deepcopy(manifest["expected_source_refs"]),
        "runtime_checker_implementation_boundary_model": {
            item: True
            for item in manifest[
                "expected_runtime_checker_implementation_boundary_model"
            ]
        },
...
def valid_payload():
    return {"runtime_checker_implementation_boundary": valid_boundary()}
...
class ManifestAndApiTests(unittest.TestCase):
    def test_manifest_is_defensive_copy(self):
        ...
        manifest["authority_summary"]["runtime_checker_runtime_authorized"] = True

        fresh = runtime_checker_implementation_boundary_validator_manifest()
    ...
    def test_public_api_and_all_exact(self):
        ...
        self.assertEqual(
            ...
            [
                "ready",
                "not_ready",
                "invalid_runtime_checker_implementation_boundary_payload",
            ],
        )
...
class HappyPathTests(unittest.TestCase):
    def test_ready_declaration_returns_ready(self):
        result = validate_runtime_checker_implementation_boundary(
            valid_payload()
        )

        self.assertIs(
            result["runtime_checker_implementation_boundary_ready"],
            True,
        )
    ...
    def test_authority_output_remains_hard_false(self):
        result = validate_runtime_checker_implementation_boundary(
            valid_payload()
        )
    ...
    def test_non_authority_output_denies_all_runtime_paths(self):
        result = validate_runtime_checker_implementation_boundary(
            valid_payload()
        )
        ...
        self.assertIs(non_authority["checker_readiness_is_runtime"], False)
        self.assertIs(
            non_authority["checker_output_is_service_admission"],
            False,
        )
...
class PayloadValidationTests(unittest.TestCase):
    def assert_failure(self, payload, failure):
        result = validate_runtime_checker_implementation_boundary(payload)
        self.assertIs(
            result["runtime_checker_implementation_boundary_ready"],
            False,
        )
    ...
    def test_top_level_shape_mismatch_fails_closed(self):
    ...
    def test_boundary_must_be_mapping(self):
        result = self.assert_failure(
    ...
    def test_boundary_shape_mismatch_fails_closed(self):
        payload = valid_payload()
        payload["runtime_checker_implementation_boundary"]["extra"] = True
        self.assert_failure(payload, "boundary_shape_mismatch")

    def test_surface_mismatch_fails_closed(self):
        payload = valid_payload()
        payload["runtime_checker_implementation_boundary"]["surface"] = "Wrong"
        self.assert_failure(payload, "boundary_surface_invalid")

    def test_version_mismatch_fails_closed(self):
        payload = valid_payload()
        payload["runtime_checker_implementation_boundary"]["version"] = 2
        self.assert_failure(payload, "boundary_version_invalid")

    def test_bool_as_int_version_rejected(self):
        payload = valid_payload()
        payload["runtime_checker_implementation_boundary"]["version"] = True
        self.assert_failure(payload, "boundary_version_invalid")

    def test_source_ref_mismatch_fails_closed(self):
        payload = valid_payload()
        payload["runtime_checker_implementation_boundary"]["source_refs"][
    ...
    def test_checker_boundary_model_invalid_fails_closed(self):
        payload = valid_payload()
        payload["runtime_checker_implementation_boundary"][
            "runtime_checker_implementation_boundary_model"
        ]["checker_runtime_forbidden"] = 1
        self.assert_failure(payload, "checker_boundary_model_invalid")

    def test_checker_enforcer_separation_invalid_fails_closed(self):
        payload = valid_payload()
        payload["runtime_checker_implementation_boundary"][
            "checker_enforcer_separation_model"
        ]["checker_must_not_execute_service_calls"] = False
    ...
    def test_forbidden_implicit_authority_invalid_fails_closed(self):
        payload = valid_payload()
        del payload["runtime_checker_implementation_boundary"][
            "forbidden_implicit_authority_sources"
        ]["future_validator_success"]
        self.assert_failure(payload, "forbidden_implicit_authority_invalid")

    def test_missing_false_authority_flag_fails_closed(self):
        payload = valid_payload()
        del payload["runtime_checker_implementation_boundary"][
            "false_authority_flags"
        ]["runtime_checker_runtime_authorized"]
    ...
    def test_missing_true_declaration_fails_closed(self):
        payload = valid_payload()
        del payload["runtime_checker_implementation_boundary"][
            "true_declarations"
    ...
    def test_true_declaration_false_fails_closed(self):
        ...
        payload["runtime_checker_implementation_boundary"][
            "true_declarations"
        ]["runtime_checker_implementation_forbidden"] = False
        self.assert_failure(payload, "required_declaration_false")

    def test_json_safety_invalid_fails_closed(self):
        payload = valid_payload()
        payload["runtime_checker_implementation_boundary"]["json_safety"][
    ...
    def test_json_safe_false_fails_closed(self):
        payload = valid_payload()
        payload["runtime_checker_implementation_boundary"]["json_safe"] = False
        self.assert_failure(payload, "json_safe_invalid")

    def test_failure_ordering_deterministic(self):
        payload = valid_payload()
        boundary = payload["runtime_checker_implementation_boundary"]
        boundary["surface"] = "Wrong"
        boundary["version"] = True
        boundary["source_refs"] = {}
        boundary["runtime_checker_implementation_boundary_model"] = {}
...
class SafetyTests(unittest.TestCase):
    def test_input_is_not_mutated(self):
        ...
        validate_runtime_checker_implementation_boundary(payload)

        self.assertEqual(payload, before)

    def test_runtime_checker_ready_does_not_authorize_actions(self):
        result = validate_runtime_checker_implementation_boundary(
            valid_payload()
        )
    ...
    def test_no_service_repository_uow_db_runtime_executor_imports(self):
        tree = ast.parse(source_text())
        imported_modules = {
            node.module
            for node in ast.walk(tree)
    ...
    def test_validator_does_not_call_adjacent_validators_or_ci(self):
        text = source_text()
        for marker in (
            "runtime_implementation_boundary_validator",
            "runtime_final_eligibility_gate_validator",
            "service_call_admission_gate_validator",
            "runtime_authority_grant_object_validator",
