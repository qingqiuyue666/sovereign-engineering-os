...
VALIDATORS = {
    ...
    "validate_generic_payload_shadow_record": validate_generic_payload_shadow_record,
    "validate_generic_payload_compatibility_gap": validate_generic_payload_compatibility_gap,
    "validate_generic_payload_shadow_report": validate_generic_payload_shadow_report,
}
...
class GenericPayloadShadowContractTests(unittest.TestCase):
    ...
    def test_policy_map_is_accepted_shadow_only(self):
        result = validate_generic_payload_shadow_policy(self.load_policy())
        self.assertTrue(result.accepted, result.failures)
        self.assertEqual(result.verdict, "accepted_shadow_only")
    ...
    def test_valid_fixtures_are_accepted(self):
        for item in self.load_fixtures()["valid_records"]:
            with self.subTest(fixture_id=item["fixture_id"]):
                ...
                self.assertEqual(result.category, item["expected_category"])
    ...
    def test_unknown_payload_becomes_compatibility_gap_not_enforcement_failure(self):
        ...
        self.assertTrue(result.accepted, result.failures)
        self.assertEqual(result.category, "compatibility_gap")
    ...
    def test_all_forbidden_flags_fail_closed_for_policy(self):
        ...
        for flag in FORBIDDEN_FLAGS:
            ...
            with self.subTest(flag=flag):
                result = validate_generic_payload_shadow_policy(payload)
                self.assertFalse(result.accepted)
    ...
    def test_high_risk_raw_keys_fail_closed(self):
        for key in ("raw_prompt", "raw_provider_response", "secret_value", "raw_traceback", "raw_exception_dump"):
            with self.subTest(key=key):
                result = validate_generic_payload_shadow_record({"shadow_mode": True, key: "forbidden"})
                self.assertFalse(result.accepted)
                self.assertIn("high_risk_payload_forbidden", result.failures)
                self.assertEqual(result.category, "high_risk_forbidden")

    def test_non_mapping_payload_raises(self):
        with self.assertRaises(GenericPayloadShadowContractViolation):
            validate_generic_payload_shadow_record(["not", "mapping"])
