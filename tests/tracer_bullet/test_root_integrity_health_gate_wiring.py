...
class RootIntegrityHealthGateWiringTests(unittest.TestCase):
    ...
    def test_health_runs_root_integrity_before_other_health_gates(self):
        ...
        expected = "health: test-root-integrity test-schemas test-tracer-bullet test-acceptance diff-check"
        self.assertEqual(health_line, expected)
        self.assertLess(health_line.index("test-root-integrity"), health_line.index("test-schemas"))
