...
def validate_final_runtime_track_map(payload: Mapping[str, Any]) -> FinalRuntimeValidationResult:
    ...
    failures: list[str] = []
    if data.get("track_type") != "seos_final_runtime_completion_track_v1":
        failures.append("track_type_invalid")
    if data.get("status") not in _ALLOWED_TRACK_STATUS:
        failures.append("track_status_invalid")
    if data.get("runtime_mode") not in _ALLOWED_RUNTIME_MODES:
        failures.append("runtime_mode_invalid")
    _reject_forbidden_true_flags(data, failures)
    if data.get("manual_preflight_required") is not True:
        failures.append("manual_preflight_required")
    if data.get("default_disabled") is not True:
        failures.append("default_disabled_required")
    stages = data.get("allowed_stages")
    if not isinstance(stages, list) or "preflight" not in stages or "receipt" not in stages:
        failures.append("allowed_stages_invalid")
    gates = data.get("required_health_gates")
    if gates != list(_REQUIRED_HEALTH_GATES):
...
def validate_runtime_preflight(payload: Mapping[str, Any]) -> FinalRuntimeValidationResult:
    ...
    if data.get("operator_approval_required") is not True:
        failures.append("operator_approval_required")
    if data.get("operator_approval_present") is not True:
        failures.append("operator_approval_missing")
    if data.get("runtime_mode") not in _ALLOWED_RUNTIME_MODES:
        failures.append("runtime_mode_invalid")
    if data.get("provider_transport_status") != "disabled":
        failures.append("provider_transport_must_be_disabled")
    for field in (
        "evidence_policy_status",
        "proof_policy_status",
        "failure_quarantine_policy_status",
        "health_gate_status",
    ):
        if data.get(field) != "passed":
...
def validate_runtime_receipt(payload: Mapping[str, Any]) -> FinalRuntimeValidationResult:
    ...
    if data.get("execution_attempted") is not False:
        failures.append("execution_attempted_must_be_false")
    if data.get("execution_mode") not in _ALLOWED_RUNTIME_MODES:
    ...
    if data.get("post_run_health_result") not in {"not_run", "passed"}:
        failures.append("post_run_health_result_invalid")
    if data.get("final_verdict") not in {"manual_dry_run_receipt_only", "blocked_fail_closed"}:
...
def validate_post_run_health(payload: Mapping[str, Any]) -> FinalRuntimeValidationResult:
    data = _require_mapping(payload)
    failures: list[str] = []
    if not isinstance(data.get("post_run_health_id"), str) or not data.get("post_run_health_id"):
        failures.append("post_run_health_id_required")
    gates = data.get("required_health_gates")
    if gates != list(_REQUIRED_HEALTH_GATES):
        failures.append("required_health_gates_invalid")
    results = data.get("gate_results")
    if not isinstance(results, Mapping):
        failures.append("gate_results_must_be_mapping")
        results = {}
    for gate in _REQUIRED_HEALTH_GATES:
        if results.get(gate) != "passed":
            failures.append(f"{gate}_not_passed")
...
def validate_failure_quarantine_link(payload: Mapping[str, Any]) -> FinalRuntimeValidationResult:
    ...
    for field in ("link_id", "task_id", "runtime_track_id", "failure_quarantine_policy_status"):
        if not isinstance(data.get(field), str) or not data.get(field):
            failures.append(f"{field}_required")
    if data.get("failure_quarantine_policy_status") != "linked_contract_only":
        failures.append("failure_quarantine_policy_status_invalid")
...
def _require_mapping(payload: Mapping[str, Any]) -> Mapping[str, Any]:
    if not isinstance(payload, Mapping):
        raise FinalRuntimeContractViolation("payload must be a mapping")
    return payload
...
def _result(failures: list[str]) -> FinalRuntimeValidationResult:
    ...
    return FinalRuntimeValidationResult(
        accepted=not unique,
