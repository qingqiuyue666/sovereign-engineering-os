...
class FinalRuntimeContractsTests(unittest.TestCase):
    ...
    def test_post_run_health_rejects_missing_gate_result(self):
        data = self.valid_post_run_health(); data["gate_results"]["test-real-merkle-proof-realization"] = "missing"; result = validate_post_run_health(data); self.assertFalse(result.accepted); self.assertIn("test-real-merkle-proof-realization_not_passed", result.failures)
