...
class FinalRuntimeContractsTests(unittest.TestCase):
    def load_track_map(self):
        self.assertTrue(TRACK_MAP_PATH.is_file(), str(TRACK_MAP_PATH))
        return json.loads(TRACK_MAP_PATH.read_text(encoding="utf-8"))

    def base_forbidden_false(self):
    ...
    def test_track_map_is_accepted_contract_only(self):
        result = validate_final_runtime_track_map(self.load_track_map())
    ...
    def test_non_mapping_payload_raises(self):
        with self.assertRaises(FinalRuntimeContractViolation):
            validate_runtime_preflight(["not", "mapping"])
