...
class LocalFixtureAdapterExecutionGatePlanTests(unittest.TestCase):
    def make_workspace(self):
        ...
        write_json_atomically(
            dry_run_path,
            self.valid_dry_run_plan_result(
                dry_output,
                fixture_path,
                receipt_path,
            ),
    ...
    def valid_dry_run_plan_result(self, dry_output, fixture_path, receipt_path):
        payload = {
            ...
            "authority": "non_authority_local_fixture_dry_run_invocation_plan_record",
            "execution_capability": "local_fixture_adapter_dry_run_invocation_plan_only",
            "invocation_plan_id": "dry-run-001",
            ...
            "local_fixture_allowed_root": receipt_path.parent.as_posix(),
            ...
            "invocation_plan_status": "local_fixture_adapter_dry_run_invocation_plan_completed",
            "invocation_plan_decision": "record_dry_run_invocation_plan_only",
            "complete": True,
            "rejected": False,
            "rejection_reasons": [],
            "next_allowed_action": "human_review_dry_run_invocation_plan_before_any_execution_gate",
            "output_dir": dry_output.as_posix(),
    ...
    def run_plan(
        ...
        *,
        ...
        return run_local_fixture_adapter_execution_gate_plan(
            ...
            operator_notes="reviewed #430 dry-run plan for execution gate",
        )

    def assert_rejects_plan_mutation(self, mutator, reason):
    ...
    def test_valid_dry_run_plan_plus_existing_fixture_writes_execution_gate_plan(self):
        ...
        self.assertEqual(
            payload["gate_plan_type"],
            "local_fixture_adapter_execution_gate_plan_v1",
        )
        self.assertEqual(payload["execution_gate_plan_id"], "gate-001")
        ...
        self.assertEqual(payload["dry_run_plan_path"], dry_run_path.as_posix())
        self.assertEqual(payload["dry_run_plan_sha256"], sha256_file(dry_run_path))
        self.assertEqual(
            payload["dry_run_plan_type"],
            "local_fixture_adapter_dry_run_invocation_plan_v1",
        )
        self.assertTrue(payload["dry_run_plan_validated"])
        ...
        self.assertTrue(payload["dry_run_plan_bound"])
        self.assertTrue(payload["execution_gate_plan_only"])
        self.assertTrue(payload["human_approval_request_only"])
        self.assertTrue(payload["execution_gate_plan_granted"])
        ...
        self.assertEqual(
            payload["execution_gate_plan_status"],
            "local_fixture_adapter_execution_gate_plan_completed",
        )
        self.assertEqual(
            payload["execution_gate_plan_decision"],
            "record_execution_gate_plan_only",
        )
        self.assertEqual(
        ...
        for path in (
            ...
            result.human_approval_request_path,
            result.summary_path,
    ...
    def test_dry_run_plan_file_shape_rejections_are_deterministic(self):
        root, dry_run_path, gate_output, _fixture_path, _usage_output = (
            self.make_workspace()
        )
        missing = root / "missing-result.json"
        result = self.run_plan(missing, gate_output)
        self.assertFalse(result.complete)
        self.assertIn("dry_run_plan_path_missing", result.rejection_reasons)

        root, dry_run_path, gate_output, _fixture_path, _usage_output = (
            self.make_workspace()
        )
        symlink_path = root / "dry-run-link.json"
        symlink_path.symlink_to(dry_run_path)
        result = self.run_plan(symlink_path, gate_output)
        self.assertFalse(result.complete)
        self.assertIn("dry_run_plan_path_is_symlink", result.rejection_reasons)

        _root, dry_run_path, gate_output, _fixture_path, _usage_output = (
            self.make_workspace()
        )
        dry_run_path.write_text("{", encoding="utf-8")
        result = self.run_plan(dry_run_path, gate_output)
        self.assertFalse(result.complete)
        self.assertIn("dry_run_plan_not_json_object", result.rejection_reasons)

    def test_required_dry_run_plan_fields_reject_fail_closed(self):
        cases = (
            (lambda payload: payload.update({"plan_type": "wrong"}), "dry_run_plan_type_mismatch"),
            (lambda payload: payload.update({"invocation_plan_status": "wrong"}), "dry_run_plan_status_not_completed"),
            (lambda payload: payload.update({"invocation_plan_decision": "wrong"}), "dry_run_plan_decision_mismatch"),
            (lambda payload: payload.update({"invocation_plan_granted": False}), "invocation_plan_not_granted"),
            (lambda payload: payload.update({"usage_receipt_validated": False}), "dry_run_plan_not_validated"),
            ...
            (lambda payload: payload.update({"usage_receipt_type": "wrong"}), "usage_receipt_type_mismatch"),
            (lambda payload: payload.update({"usage_receipt_sha256": ""}), "usage_receipt_sha_missing"),
            ...
            (lambda payload: payload.update({"one_usage_receipt_bound": False}), "one_usage_receipt_bound_not_true"),
            (lambda payload: payload.update({"dry_run_plan_only": False}), "dry_run_plan_only_not_true"),
            (lambda payload: payload.update({"future_invocation_requires_separate_execution_gate": False}), "future_execution_gate_not_required"),
            (lambda payload: payload.update({"future_execution_requires_human_approval": False}), "future_human_approval_not_required"),
            ...
            (lambda payload: payload.update({"approval_token_issued": True}), "approval_material_claimed"),
            (lambda payload: payload.update({"auto_approval_performed": True}), "approval_material_claimed"),
            (lambda payload: payload.update({"runnable_job_created": True}), "runnable_job_claimed"),
        )
        for mutator, reason in cases:
            with self.subTest(reason=reason):
                self.assert_rejects_plan_mutation(mutator, reason)

    def test_missing_performed_and_materialized_false_fields_reject(self):
        for field_name in (
            ...
            with self.subTest(field_name=field_name):
                self.assert_rejects_plan_mutation(
        ...
        for field_name in (
            ...
            with self.subTest(field_name=field_name):
                self.assert_rejects_plan_mutation(
                    lambda payload, field_name=field_name: payload.pop(field_name),
                    "executable_material_claimed",
                )

    def test_local_fixture_plan_metadata_rejections_are_deterministic(self):
        cases = (
    ...
    def test_widened_dry_run_plan_output_dir_cannot_admit_outside_fixture(self):
        root, dry_run_path, gate_output, _fixture_path, _usage_output = (
            self.make_workspace()
        )
    ...
    def test_dry_run_manifest_and_artifact_index_hash_mismatch_reject(self):
        _root, dry_run_path, gate_output, _fixture_path, dry_output = (
            self.make_workspace()
        )
        manifest_path = dry_run_path.parent / "local_fixture_adapter_dry_run_invocation_plan_manifest.json"
        write_json(
            manifest_path,
            {
                "result_path": dry_run_path.as_posix(),
                "result_sha256": "0" * 64,
            },
        )
        result = self.run_plan(dry_run_path, gate_output)
        self.assertFalse(result.complete)
        self.assertIn("dry_run_manifest_hash_mismatch", result.rejection_reasons)

        _root, dry_run_path, gate_output, _fixture_path, _usage_output = (
            self.make_workspace()
        )
        artifact_index_path = dry_run_path.parent / "artifact_index.json"
        write_json(
            artifact_index_path,
            {
                "entries": [
                    {
                        "path": dry_run_path.as_posix(),
                        "sha256": "0" * 64,
        ...
        result = self.run_plan(dry_run_path, gate_output)
        self.assertFalse(result.complete)
        self.assertIn("dry_run_artifact_index_hash_mismatch", result.rejection_reasons)
    ...
    def test_output_collision_rejects_without_overwrite(self):
        ...
        (
            gate_output / "local_fixture_adapter_execution_gate_plan_result.json"
        ).write_text("{}\n", encoding="utf-8")
    ...
    def test_successful_output_preserves_denials_future_blocks_and_no_runner(self):
        _root, dry_run_path, gate_output, _fixture_path, _usage_output = (
            self.make_workspace()
        )
        ...
        self.assertFalse(payload["approval_token_issued"])
        self.assertFalse(payload["auto_approval_performed"])
        self.assertFalse(payload["runnable_job_created"])
        self.assertFalse(payload["execution_runner_created"])
        self.assertTrue(payload["future_execution_requires_separate_human_approval_artifact"])
        self.assertTrue(payload["future_execution_requires_separate_execution_runner_pr"])
    ...
    def test_human_approval_request_contains_warning_language_and_hashes(self):
        ...
        required_fragments = (
            ...
            "Next allowed action: human_review_execution_gate_plan_before_any_runner_implementation",
        )
        for fragment in required_fragments:
            self.assertIn(fragment, text)

    def test_human_approval_request_contains_no_executable_material(self):
        _root, dry_run_path, gate_output, _fixture_path, _usage_output = (
            self.make_workspace()
        )

        result = self.run_plan(dry_run_path, gate_output)
        text = result.human_approval_request_path.read_text(encoding="utf-8").lower()
    ...
    def test_cli_accepts_required_args_and_emits_execution_gate_plan(self):
        _root, dry_run_path, gate_output, _fixture_path, _usage_output = (
            self.make_workspace()
        )
        stdout = io.StringIO()
    ...
    def test_launcher_accepts_required_args_and_emits_execution_gate_plan(self):
        _root, dry_run_path, gate_output, _fixture_path, _usage_output = (
            self.make_workspace()
        )
    ...
    def test_task_graph_accepts_execution_gate_plan_node_and_writes_artifacts(self):
        root, dry_run_path, gate_output, _fixture_path, _usage_output = (
            self.make_workspace()
        )
        ...
        write_json_atomically(
            graph_path,
            {
                ...
                "graph_id": "execution-gate-plan-graph",
                ...
                "nodes": [
                    {
                        ...
                        "capability": "launch_local_fixture_adapter_execution_gate_plan",
                        "execution_mode": "fixture",
                        "depends_on": [],
                        "approval_checkpoint_required": True,
                        "inputs": {
                            "dry_run_plan": dry_run_path.as_posix(),
                            "output_dir": gate_output.as_posix(),
                            "execution_gate_plan_id": "gate-graph",
                            "review_attestation": LOCAL_FIXTURE_ADAPTER_EXECUTION_GATE_PLAN_REVIEW_ATTESTATION,
                        },
                    }
                ],
        ...
        self.assertTrue(node["local_fixture_adapter_execution_gate_plan_complete"])
        roles = {
            artifact["artifact_role"]
            for artifact in artifact_outputs["artifacts"]
            if artifact["node_id"] == "gate_plan"
        }
        self.assertIn("local_fixture_adapter_execution_gate_plan_result", roles)
        self.assertIn("local_fixture_adapter_execution_gate_plan_plan", roles)
        self.assertIn(
    ...
    def test_adapter_registry_includes_execution_gate_plan_candidate_local_fixture(self):
        entry = find_adapter_entry("local_fixture_adapter_execution_gate_plan")
    ...
    def test_product_health_check_includes_execution_gate_without_live_admission(self):
    ...
    def test_decision_doc_exists_and_states_execution_gate_boundary(self):
        text = " ".join(DECISION_DOC.read_text(encoding="utf-8").split())
        required_fragments = (
            "execution-gate plan",
            "does not approve execution",
            "does not issue approval token",
            "does not create an execution runner",
            "does not execute the adapter",
            "does not execute Playwright",
            "does not open a browser",
            "does not execute #422/#424/#425/#426/#427/#428/#429/#430",
            ...
            "validates the #430 dry-run plan and local fixture file path/hash",
            "non-executable execution-gate plan plus human approval request",
