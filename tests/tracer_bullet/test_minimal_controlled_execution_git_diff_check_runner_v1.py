"""Tracer-bullet tests for Minimal Controlled Execution Git Diff Check Runner V1."""
...
from kernel.execution import minimal_controlled_git_diff_check_runner as runner
...
SOURCE_PATH = Path("kernel/execution/minimal_controlled_git_diff_check_runner.py")


def _request_payload(command_id: str = "git_diff_check", **overrides: object) -> dict[str, object]:
    payload: dict[str, object] = {
        "approval_token_id": "",
        "caller_intent": "execute the minimal git diff check slice",
...
def _run_success(stdout: str = "") -> runner.MinimalControlledExecutionRunResult:
    with mock.patch.object(runner.subprocess, "run", return_value=_completed(stdout=stdout)):
        return runner.run_minimal_controlled_git_diff_check(_request_payload())


class MinimalControlledExecutionGitDiffCheckRunnerV1Tests(unittest.TestCase):
    def test_git_diff_check_executes_with_fixed_argv_only(self):
        with mock.patch.object(runner.subprocess, "run", return_value=_completed()) as run:
            result = runner.run_minimal_controlled_git_diff_check(_request_payload())

        self.assertEqual(result.receipt.command_id, "git_diff_check")
        ...
        self.assertEqual(
            ...
            INITIAL_COMMAND_REGISTRY["git_diff_check"].timeout_ms / 1000,
        )
        self.assertEqual(result.attempt.argv, ("git", "diff", "--check"))

    def test_subprocess_receives_required_git_safe_env(self):
        with mock.patch.object(runner.subprocess, "run", return_value=_completed()) as run:
            runner.run_minimal_controlled_git_diff_check(_request_payload())
        ...
        self.assertEqual(
            env,
            {
                ...
                "HOME": str(runner.REPOSITORY_ROOT),
                "LANG": "C",
                "LC_ALL": "C",
        ...
        for field_name in sorted(required_fields):
            ...
            with self.subTest(field_name=field_name):
                with self.assertRaisesRegex(ValueError, "forbidden_request_field"):
                    runner.run_minimal_controlled_git_diff_check(payload)

    def test_git_status_short_is_not_executable_in_this_slice(self):
        with mock.patch.object(runner.subprocess, "run") as run:
            result = runner.run_minimal_controlled_git_diff_check(
    ...
    def test_unknown_command_id_is_rejected(self):
        with mock.patch.object(runner.subprocess, "run") as run:
            result = runner.run_minimal_controlled_git_diff_check(_request_payload("unknown_command"))
    ...
    def test_raw_stdout_and_stderr_are_not_stored_in_receipt_or_wal(self):
        raw_stdout = "raw diff text"
        raw_stderr = "raw warning text"
    ...
    def test_timeout_creates_failure_bundle(self):
        timeout = runner.subprocess.TimeoutExpired(
            cmd=["git", "diff", "--check"],
            timeout=INITIAL_COMMAND_REGISTRY["git_diff_check"].timeout_ms / 1000,
        )
    ...
    def test_nonzero_exit_creates_failure_bundle(self):
        with mock.patch.object(
            runner.subprocess,
            "run",
            return_value=_completed(returncode=1, stderr="whitespace error"),
        ):
            result = runner.run_minimal_controlled_git_diff_check(_request_payload())
    ...
    def test_output_limit_exceeded_creates_truncation_and_failure_evidence(self):
        limit = INITIAL_COMMAND_REGISTRY["git_diff_check"].output_limit_bytes
        with mock.patch.object(
            runner.subprocess,
            "run",
    ...
    def test_verifier_rejects_tampered_receipt_hash(self):
        ...
        verification = runner.verify_minimal_controlled_git_diff_check_result(
    ...
    def test_executable_slice_contains_exactly_git_diff_check(self):
        self.assertEqual(runner.EXECUTABLE_COMMAND_IDS, ("git_diff_check",))
