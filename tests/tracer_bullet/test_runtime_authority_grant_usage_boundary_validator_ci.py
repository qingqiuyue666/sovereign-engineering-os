...
from kernel.lifecycle import (
    runtime_authority_grant_usage_boundary_validator_ci as module,
)
from kernel.lifecycle.runtime_authority_grant_usage_boundary_validator_ci import (
...
class LeakyValue:
    def __repr__(self):
        return "LEAKY_GRANT_USAGE_CI_OBJECT"


def manifest():
    return runtime_authority_grant_usage_boundary_validator_ci_manifest()
...
def invalid_validator_payload(failure="payload_not_mapping"):
    ...
    payload["reason_code"] = INVALID_VALIDATOR_PAYLOAD
    return payload
...
def assert_no_leak(testcase, result):
    ...
    testcase.assertNotIn("LEAKY_GRANT_USAGE_CI_OBJECT", rendered)
    testcase.assertNotIn("LEAKY_GRANT_USAGE_CI_OBJECT", repr(result))
...
class ManifestAndApiTests(unittest.TestCase):
    ...
    def test_no_validator_import_or_call(self):
        ...
        for imported in imported_modules:
            self.assertNotIn(
                "runtime_authority_grant_usage_boundary_validator",
                imported,
            )
        ...
        self.assertFalse(any(name.startswith("validate_") for name in calls))
        self.assertFalse(
            any(name.startswith("consume_runtime_") for name in calls)
        )


class HappyPathTests(unittest.TestCase):
    def test_valid_ready_validator_output_returns_ci_ok_true(self):
        result = consume_runtime_authority_grant_usage_boundary_validator_ci(
            valid_payload()
        )
    ...
    def test_ci_ok_authorizes_no_runtime_grant_service_db_or_append(self):
        result = consume_runtime_authority_grant_usage_boundary_validator_ci(
            valid_payload()
        )
        ...
        self.assertIs(
            non_authority["ci_success_is_grant_usage_authority"],
            False,
        )
        self.assertIs(
            non_authority["ci_success_is_authority_ref_usage_authority"],
            False,
        )
        self.assertIs(
            non_authority["ci_success_is_service_call_authority"],
            False,
        )
        self.assertIs(non_authority["ci_success_is_db_write_authority"], False)
        self.assertIs(non_authority["ci_success_is_append_authority"], False)
        self.assertIs(
            non_authority["ci_success_is_transaction_authority"],
            False,
        )
        self.assertIs(non_authority["ci_success_is_executor_authority"], False)
        self.assertIs(
            non_authority["ci_success_is_durable_write_authority"],
            False,
        )
        self.assertIs(non_authority["ci_success_is_physical_io_authority"], False)

    def test_grant_usage_authority_ref_admission_executor_physical_io_non_authority(self):
        result = consume_runtime_authority_grant_usage_boundary_validator_ci(
            valid_payload()
        )
        non_authority = result["non_authority"]

        self.assertIs(non_authority["grant_usage_remains_non_authorized"], True)
        self.assertIs(
            non_authority["grant_consumption_remains_non_authorized"],
            True,
        )
        self.assertIs(
            non_authority["grant_validation_runtime_remains_non_authorized"],
            True,
        )
        self.assertIs(
            non_authority[
                "grant_revocation_enforcement_remains_non_authorized"
            ],
            True,
        )
        self.assertIs(
            non_authority[
                "authority_ref_runtime_usage_remains_non_authorized"
            ],
            True,
        )
        self.assertIs(
            non_authority[
                "service_call_admission_remains_non_authorized"
            ],
            True,
        )
        self.assertIs(
            non_authority["admission_decision_remains_non_authorized"],
            True,
        )
        self.assertIs(non_authority["service_calls_remain_non_authorized"], True)
        self.assertIs(non_authority["db_writes_remain_non_authorized"], True)
        self.assertIs(
            non_authority["evidence_audit_append_remains_non_authorized"],
            True,
        )
        self.assertIs(
            non_authority[
                "transaction_idempotency_rollback_remains_non_authorized"
            ],
            True,
        )
        self.assertIs(
            non_authority["executor_dispatch_remains_non_authorized"],
            True,
        )
        self.assertIs(non_authority["durable_writes_remain_non_authorized"], True)
        self.assertIs(non_authority["physical_io_remains_non_authorized"], True)
...
class PayloadValidationTests(unittest.TestCase):
    ...
    def test_failure_list_rejects_non_string_without_echo(self):
        ...
        self.assertNotIn("LEAKY_GRANT_USAGE_CI_OBJECT", repr(result))
    ...
    def test_boundary_source_refs_mismatch(self):
        payload = valid_payload()
    ...
    def test_boundary_model_mismatch(self):
        ...
        ]["grant_object_existence_boundary"] = False
    ...
    def test_reason_code_mapping(self):
        ready = consume_runtime_authority_grant_usage_boundary_validator_ci(
            valid_payload()
        )
        not_ready = consume_runtime_authority_grant_usage_boundary_validator_ci(
            not_ready_payload("source_ref_mismatch")
        )
        invalid = consume_runtime_authority_grant_usage_boundary_validator_ci(
            []
        )
    ...
    def test_input_immutability(self):
        ...
        consume_runtime_authority_grant_usage_boundary_validator_ci(payload)
...
class SourceBoundaryTests(unittest.TestCase):
            ...
            if isinstance(node, ast.Import):
                ...
                self.assertFalse(
                    module_name.startswith(
                        "kernel.lifecycle.runtime_authority_grant_usage_"
                        "boundary_validator"
                    )
                )
    ...
    def test_no_service_db_repository_uow_audit_evidence_executor_calls(self):
        ...
        for module_name in imported:
            ...
            self.assertNotIn("recovery", module_name)
            self.assertNotIn("cli", module_name.lower())
