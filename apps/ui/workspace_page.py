"""Mission-centered default workspace for Sovereign Console."""
...
class WorkspacePage(QWidget):
    """Unified work surface; it displays projections and disabled command intents."""
    ...
    def __init__(self, parent: object | None = None, *, language: str = "en") -> None:
        ...
        for index, field in enumerate(MISSION_FIELD_KEYS):
            ...
            mission_grid.addWidget(label, index, 0)
            mission_grid.addWidget(value, index, 1)
        mission_layout.addLayout(mission_grid)
        top_row.addWidget(self.mission_card, stretch=2)

        self.command_panel = QFrame(self)
        self.command_panel.setObjectName("WorkspaceCommandPanel")
        self.command_panel.setProperty("role", "metricCard")
        ...
        command_layout.addWidget(_eyebrow(self.text.tr("common.next_action"), self.command_panel))
        command_layout.addWidget(self.next_action_label)
        command_layout.addStretch(1)
        top_row.addWidget(self.command_panel, stretch=1)
        ...
        layout.addWidget(self.chain_panel)
