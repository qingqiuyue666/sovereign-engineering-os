...
def run_model_fixture(
    ...
    try:
        ...
        output = _build_or_parse_fixture_output(request, input_artifact)
        _validate_model_output(request["schema_name"], output)
...
def _validate_request(request):
    ...
    if request.get("provider") != _PROVIDER:
        raise ValueError("live model provider is not admitted")
    if request.get("schema_name") not in ModelFixtureSchema.allowed():
        raise ValueError("schema_name is not supported")
    if "api_key" in request or request.get("api_key_required") is True:
