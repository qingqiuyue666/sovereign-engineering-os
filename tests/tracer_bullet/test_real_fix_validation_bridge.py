...
class RealFixValidationBridgeTest(unittest.TestCase):
    ...
    def test_bridge_produces_one_schema_valid_receipt_linked_to_both_sides(
        ...
        # upstream ``validation_receipt_created`` record emitted by
        # ``ValidationService.validate`` must also name the same durable
        # intent-anchor id in both ``payload`` and ``artifact_refs``.
        # The service performs no independent verification;
        # authoritative fail-closed verification of ``intent_id``
        # against ``intent_anchor_records`` remains the responsibility
        # of ``RevisionSealService`` downstream. Naming it on the
        # service record closes the validation-stage one-hop asymmetry
        # next to the already-closed bridge-layer
        # ``real_fix_validation_bridge_attested``.
        created = self._records_of("validation_receipt_created")
        self.assertEqual(len(created), 1)
        created_rec = created[0]
        self.assertEqual(created_rec["task_id"], task_id)
        self.assertEqual(created_rec["payload"]["intent_id"], expected_intent_id)
        self.assertIn(expected_intent_id, created_rec["artifact_refs"])
