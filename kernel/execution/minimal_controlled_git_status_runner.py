"""Minimal controlled runner for the git_status_short execution slice."""
...
__all__ = [
    ...
    "verify_minimal_controlled_git_status_result",
]

REPOSITORY_ROOT = Path(__file__).resolve().parents[2]
EXECUTABLE_COMMAND_IDS = ("git_status_short",)
RUNNER_FAILURE_TYPES = frozenset(
    set(EXECUTION_FAILURE_TYPES) | {"COMMAND_NOT_EXECUTABLE_IN_THIS_SLICE"}
)
_COMMAND_ID = "git_status_short"
...
class MinimalControlledExecutionVerifierBinding(_DictMixin):
    ...
    def __post_init__(self) -> None:
        _install_hash(self, "verifier_binding_hash", verifier_binding_hash)
...
def run_minimal_controlled_git_status(
...
def verify_minimal_controlled_git_status_result(
    ...
    _check(tuple(INITIAL_COMMAND_REGISTRY) == ("git_status_short", "git_diff_check"), failures, "registry_command_ids_mismatch")
    _check(EXECUTABLE_COMMAND_IDS == ("git_status_short",), failures, "executable_command_ids_mismatch")
    _check(result.pre_snapshot.snapshot_hash == snapshot_hash(result.pre_snapshot), failures, "pre_snapshot_hash_mismatch")
    _check(result.post_snapshot.snapshot_hash == snapshot_hash(result.post_snapshot), failures, "post_snapshot_hash_mismatch")
    ...
    if receipt is not None:
        ...
        _check(receipt.policy_decision_hash == decision.decision_hash, failures, "policy_decision_hash_mismatch")
        _check(receipt.registry_entry_hash == entry.registry_entry_hash, failures, "registry_entry_hash_mismatch")
        _check(receipt.pre_snapshot_hash == result.pre_snapshot.snapshot_hash, failures, "pre_snapshot_hash_mismatch")
        _check(receipt.post_snapshot_hash == result.post_snapshot.snapshot_hash, failures, "post_snapshot_hash_mismatch")
        _check(receipt.output_limit_bytes == entry.output_limit_bytes, failures, "output_limit_bytes_mismatch")
        ...
        if result.attempt is None:
            ...
            _check(result.attempt.env_keys == tuple(sorted(_MINIMAL_ENV)), failures, "attempt_env_mismatch")
...
def _result(
    ...
    verification = verify_minimal_controlled_git_status_result(result)
