...
VALIDATORS = {
    "validate_protected_storage_policy": validate_protected_storage_policy,
    "validate_storage_manifest": validate_storage_manifest,
    "validate_access_policy": validate_access_policy,
    "validate_recovery_policy": validate_recovery_policy,
    "validate_deletion_policy": validate_deletion_policy,
    "validate_storage_contract_report": validate_storage_contract_report,
}
...
class ProtectedEvidenceStorageContractTests(unittest.TestCase):
    ...
    def test_policy_map_is_accepted_contract_only(self):
        result = validate_protected_storage_policy(self.load_policy())
    ...
    def test_all_forbidden_flags_fail_closed_for_policy(self):
        ...
        for flag in FORBIDDEN_FLAGS:
            ...
            with self.subTest(flag=flag):
                result = validate_protected_storage_policy(payload)
                self.assertFalse(result.accepted)
    ...
    def test_manifest_access_recovery_and_deletion_are_required(self):
        ...
        for flag in (
            ...
            with self.subTest(flag=flag):
                result = validate_protected_storage_policy(payload)
                self.assertFalse(result.accepted)
                self.assertIn(f"{flag}_missing", result.failures)
    ...
    def test_access_policy_rejects_plaintext_and_key_access(self):
        ...
        result = validate_access_policy(payload)
        self.assertFalse(result.accepted)
        self.assertIn("plaintext_access_must_be_false", result.failures)
        self.assertIn("key_material_access_must_be_false", result.failures)

    def test_report_requires_all_sections_and_no_implementation_ready(self):
        payload = copy.deepcopy(self.valid_by_validator()["validate_storage_contract_report"])
        payload["validated_sections"] = ["policy"]
        payload["implementation_ready"] = True
        result = validate_storage_contract_report(payload)
        self.assertFalse(result.accepted)
        self.assertIn("storage_manifest_section_missing", result.failures)
        self.assertIn("access_policy_section_missing", result.failures)
        self.assertIn("recovery_policy_section_missing", result.failures)
        self.assertIn("deletion_policy_section_missing", result.failures)
        self.assertIn("implementation_ready_must_be_false", result.failures)

    def test_non_mapping_payload_raises(self):
        with self.assertRaises(ProtectedEvidenceStorageContractViolation):
            validate_protected_storage_policy(["not", "mapping"])

    def test_makefile_declares_protected_storage_gate(self):
        text = Path("Makefile").read_text(encoding="utf-8")
        self.assertIn("test-protected-evidence-storage", text)
        self.assertIn(
    ...
    def test_runbook_exists_and_records_contract_boundary(self):
        path = Path("docs/runbooks/protected_evidence_storage_contract_v1.md")
