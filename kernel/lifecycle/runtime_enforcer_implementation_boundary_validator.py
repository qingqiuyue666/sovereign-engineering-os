"""Read-only validator for RuntimeEnforcerImplementationBoundaryV1 declarations."""
...
_SURFACE = "runtime_enforcer_implementation_boundary_validator"
_CONTRACT_NAME = "RuntimeEnforcerImplementationBoundaryV1"
_VERSION = 1

_READY = "ready"
_NOT_READY = "not_ready"
_INVALID_PAYLOAD = "invalid_runtime_enforcer_implementation_boundary_payload"

_INPUT_KEY = "runtime_enforcer_implementation_boundary"
...
_MANIFEST = {
    ...
    "package": "runtime-enforcer-implementation-boundary-validator-v1",
    ...
    "expected_runtime_enforcer_implementation_boundary_model": list(
        _RUNTIME_ENFORCER_IMPLEMENTATION_BOUNDARY_MODEL
    ),
...
def runtime_enforcer_implementation_boundary_validator_manifest():
    """Return bounded metadata for the read-only structural validator."""

    return deepcopy(_MANIFEST)


def validate_runtime_enforcer_implementation_boundary(payload):
    """Validate an already-rendered RuntimeEnforcerImplementationBoundaryV1."""
...
def _validate_boundary(boundary, failures):
    ...
    _validate_true_mapping(
        boundary.get("runtime_enforcer_implementation_boundary_model"),
        _RUNTIME_ENFORCER_IMPLEMENTATION_BOUNDARY_MODEL,
        "enforcer_boundary_model_invalid",
        "enforcer_boundary_model_invalid",
        failures,
...
def _result(failures):
    ...
    if ready:
        reason_code = _READY
    elif "payload_not_mapping" in ordered_failures:
        reason_code = _INVALID_PAYLOAD
    else:
        reason_code = _NOT_READY

    return {
        "runtime_enforcer_implementation_boundary_ready": ready,
