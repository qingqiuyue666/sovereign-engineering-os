"""Read-only CI consumer for rendered checker-enforcer boundary output."""
...
_SURFACE = "runtime_checker_enforcer_separation_boundary_validator_ci"
...
_PAYLOAD_KEYS = (
    "runtime_checker_enforcer_separation_boundary_ready",
    "reason_code",
    "failures",
    "boundary",
    "authority",
...
_EXPECTED_BOUNDARY = {
    ...
    "checker_enforcer_separation_boundary_model": {
        item: True for item in _CHECKER_ENFORCER_SEPARATION_BOUNDARY_MODEL
    },
    "false_authority_flags": {
...
_CI_NON_AUTHORITY_SUMMARY = {
    ...
    "enforcer_output_is_durable_write_authorization": False,
    "toctou_physical_enforcement_is_implemented": False,
    "execution_context_binding_is_implemented": False,
    "final_pre_dispatch_context_validation_is_implemented": False,
    "payload_environment_filesystem_binding_is_implemented": False,
    "digest_merkle_environment_snapshot_is_implemented": False,
    "ghost_authority_is_permitted": False,
    "stale_dead_intent_authority_is_permitted": False,
    "capability_token_work_is_implemented": False,
    "forensic_audit_binding_is_implemented": False,
    "evidence_audit_bookkeeping_is_implemented": False,
    "transaction_idempotency_is_implemented": False,
    "executor_dispatch_is_implemented": False,
    "durable_write_final_authorization_is_implemented": False,
    "pid_time_audit_binding_is_implemented": False,
    "sqlite_uow_audit_append_is_implemented": False,
}
...
_MANIFEST = {
    ...
    "package": "runtime-checker-enforcer-separation-boundary-validator-ci-v1",
...
def runtime_checker_enforcer_separation_boundary_validator_ci_manifest():
    """Return bounded metadata for the read-only CI consumer."""

    return deepcopy(_MANIFEST)


def consume_runtime_checker_enforcer_separation_boundary_validator_ci(payload):
...
def _matches_exact_json(candidate, expected):
    ...
    if type(expected) is list:
        if type(candidate) is not list or len(candidate) != len(expected):
            return False
        for item, expected_item in zip(candidate, expected):
            if not _matches_exact_json(item, expected_item):
                return False
        return True
