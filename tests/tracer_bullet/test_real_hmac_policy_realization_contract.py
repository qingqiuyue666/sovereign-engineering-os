...
class RealHmacPolicyRealizationContractTests(unittest.TestCase):
    ...
    def test_all_forbidden_flags_fail_closed_for_policy(self):
        ...
        for flag in FORBIDDEN_FLAGS:
            ...
            with self.subTest(flag=flag):
                result = validate_real_hmac_policy(payload)
                self.assertFalse(result.accepted)
    ...
    def test_allowed_digest_algorithms_are_strict(self):
        ...
        payload["allowed_digest_algorithms"] = ["sha256", "sha384", "sha512", "md5"]
        result = validate_real_hmac_policy(payload)
        self.assertFalse(result.accepted)
    ...
    def test_verifier_requires_constant_time_compare_before_real_verification(self):
        payload = copy.deepcopy(self.valid_by_validator()["validate_hmac_verifier_policy"])
        payload["constant_time_compare_required"] = False
        payload["verification_runtime_allowed"] = True
        result = validate_hmac_verifier_policy(payload)
        self.assertFalse(result.accepted)
        self.assertIn("constant_time_compare_required_missing", result.failures)
        self.assertIn("verification_runtime_allowed_must_be_false", result.failures)

    def test_report_requires_all_sections_and_no_real_signature_ready(self):
        payload = copy.deepcopy(self.valid_by_validator()["validate_hmac_policy_realization_report"])
        payload["validated_sections"] = ["policy"]
        payload["real_signature_ready"] = True
        result = validate_hmac_policy_realization_report(payload)
        self.assertFalse(result.accepted)
        self.assertIn("key_policy_section_missing", result.failures)
        self.assertIn("signing_authority_policy_section_missing", result.failures)
        self.assertIn("verifier_policy_section_missing", result.failures)
        self.assertIn("signature_receipt_contract_section_missing", result.failures)
        self.assertIn("rotation_policy_section_missing", result.failures)
        self.assertIn("real_signature_ready_must_be_false", result.failures)

    def test_non_mapping_payload_raises(self):
        with self.assertRaises(RealHmacPolicyContractViolation):
            validate_real_hmac_policy(["not", "mapping"])
