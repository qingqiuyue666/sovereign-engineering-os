...
class RealMerkleProofRealizationContractTests(unittest.TestCase):
    ...
    def test_policy_map_is_accepted_contract_only(self):
        result = validate_real_merkle_policy(self.load_policy())
    ...
    def test_all_forbidden_flags_fail_closed_for_policy(self):
        ...
        for flag in FORBIDDEN_FLAGS:
            ...
            with self.subTest(flag=flag):
                result = validate_real_merkle_policy(payload)
                self.assertFalse(result.accepted)
                self.assertIn(f"{flag}_forbidden", result.failures)

    def test_allowed_digest_algorithms_are_strict(self):
        payload = self.load_policy()
        payload["allowed_digest_algorithms"] = ["sha256", "sha512", "md5"]
        result = validate_real_merkle_policy(payload)
        self.assertFalse(result.accepted)
        self.assertIn("allowed_digest_algorithms_invalid", result.failures)

    def test_leaf_and_proof_reject_raw_payloads(self):
        leaf = copy.deepcopy(self.valid_by_validator()["validate_merkle_leaf_digest_policy"])
        leaf["raw_leaf_payload_allowed"] = True
        leaf_result = validate_merkle_leaf_digest_policy(leaf)
        self.assertFalse(leaf_result.accepted)
        self.assertIn("raw_leaf_payload_allowed_must_be_false", leaf_result.failures)
        proof = copy.deepcopy(self.valid_by_validator()["validate_merkle_proof_path_contract"])
        proof["raw_sibling_payload_allowed"] = True
        proof_result = validate_merkle_proof_path_contract(proof)
        self.assertFalse(proof_result.accepted)
        self.assertIn("raw_sibling_payload_allowed_must_be_false", proof_result.failures)

    def test_report_requires_all_sections_and_no_real_tree_ready(self):
        payload = copy.deepcopy(self.valid_by_validator()["validate_merkle_policy_realization_report"])
        payload["validated_sections"] = ["policy"]
        payload["real_tree_ready"] = True
        result = validate_merkle_policy_realization_report(payload)
        self.assertFalse(result.accepted)
        self.assertIn("tree_manifest_section_missing", result.failures)
        self.assertIn("leaf_digest_policy_section_missing", result.failures)
        self.assertIn("node_digest_policy_section_missing", result.failures)
        self.assertIn("proof_path_contract_section_missing", result.failures)
        self.assertIn("verifier_receipt_contract_section_missing", result.failures)
        self.assertIn("root_commitment_receipt_contract_section_missing", result.failures)
        self.assertIn("real_tree_ready_must_be_false", result.failures)

    def test_non_mapping_payload_raises(self):
        with self.assertRaises(RealMerkleProofContractViolation):
            validate_real_merkle_policy(["not", "mapping"])

    def test_makefile_declares_real_merkle_proof_realization_gate(self):
        text = Path("Makefile").read_text(encoding="utf-8")
        self.assertIn("test-real-merkle-proof-realization", text)
        self.assertIn("PYTHONDONTWRITEBYTECODE=1 $(PYTHON) -m unittest tests.tracer_bullet.test_real_merkle_proof_realization_contract -v", text)
    ...
    def test_runbook_exists_and_records_merkle_boundary(self):
        path = Path("docs/runbooks/real_merkle_proof_realization_contract_v1.md")
