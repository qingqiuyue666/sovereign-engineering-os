...
VALIDATORS = {"validate_runtime_sealed_receipt_policy": validate_runtime_sealed_receipt_policy, "validate_transport_attempt_receipt": validate_transport_attempt_receipt, "validate_blocked_attempt_receipt": validate_blocked_attempt_receipt, "validate_postcheck_receipt": validate_postcheck_receipt, "validate_receipt_chain": validate_receipt_chain}
FORBIDDEN_FLAGS = ("provider_live_call_performed", "transport_attempted", "network_accessed", "secret_value_read", "secret_value_persisted", "env_read_performed", "raw_prompt_persisted", "raw_response_persisted", "sqlite_schema_changed", "audit_append_performed", "protected_storage_implemented", "real_hmac_performed", "real_merkle_tree_built", "zero_knowledge_proof_built", "production_autonomy_enabled", "raw_evidence_store_allowed")
...
class RuntimeSealedReceiptContractsTests(unittest.TestCase):
    ...
    def test_policy_map_is_accepted_contract_only(self):
        result = validate_runtime_sealed_receipt_policy(self.load_policy()); self.assertTrue(result.accepted, result.failures); self.assertEqual(result.verdict, "accepted_contract_only")
    ...
    def test_receipt_chain_requires_all_links(self):
        payload = copy.deepcopy(self.valid_by_validator()["validate_receipt_chain"]); payload["policy_receipt_ref"] = ""; payload["chain_complete"] = False; result = validate_receipt_chain(payload); self.assertIn("policy_receipt_ref_required", result.failures); self.assertIn("chain_complete_required", result.failures)
    def test_transport_receipt_requires_seal_postcheck_and_quarantine_link(self):
        payload = copy.deepcopy(self.valid_by_validator()["validate_transport_attempt_receipt"]); payload["sealed"] = False; payload["postcheck_required"] = False; payload["failure_quarantine_link_required"] = False; result = validate_transport_attempt_receipt(payload); self.assertIn("sealed_required", result.failures); self.assertIn("postcheck_required", result.failures); self.assertIn("failure_quarantine_link_required", result.failures)
    def test_non_mapping_payload_raises(self):
        with self.assertRaises(RuntimeSealedReceiptContractViolation): validate_runtime_sealed_receipt_policy(["not", "mapping"])
    def test_makefile_declares_runtime_sealed_receipt_gate(self):
        text = Path("Makefile").read_text(encoding="utf-8"); self.assertIn("test-runtime-sealed-receipt", text); self.assertIn(EXPECTED_HEALTH, text); self.assertLess(EXPECTED_HEALTH.index("test-protected-evidence-storage"), EXPECTED_HEALTH.index("test-protected-evidence-storage-implementation")); self.assertLess(EXPECTED_HEALTH.index("test-protected-evidence-storage-implementation"), EXPECTED_HEALTH.index("test-real-hmac-policy-realization")); self.assertLess(EXPECTED_HEALTH.index("test-real-merkle-proof-realization"), EXPECTED_HEALTH.index("test-generic-payload-full-enforcement")); self.assertLess(EXPECTED_HEALTH.index("test-generic-payload-full-enforcement"), EXPECTED_HEALTH.index("test-schemas"))
