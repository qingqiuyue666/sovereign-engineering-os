...
class GatedProviderTransportContractsTests(unittest.TestCase):
    ...
    def test_makefile_declares_gated_provider_transport_gate(self):
        text = Path("Makefile").read_text(encoding="utf-8"); self.assertIn("test-gated-provider-transport", text); self.assertIn(EXPECTED_HEALTH, text); self.assertLess(EXPECTED_HEALTH.index("test-gated-provider-transport"), EXPECTED_HEALTH.index("test-real-runtime-provider-transport-execution")); self.assertLess(EXPECTED_HEALTH.index("test-real-runtime-provider-transport-execution"), EXPECTED_HEALTH.index("test-runtime-sealed-receipt")); self.assertLess(EXPECTED_HEALTH.index("test-protected-evidence-storage"), EXPECTED_HEALTH.index("test-protected-evidence-storage-implementation")); self.assertLess(EXPECTED_HEALTH.index("test-protected-evidence-storage-implementation"), EXPECTED_HEALTH.index("test-real-hmac-policy-realization")); self.assertLess(EXPECTED_HEALTH.index("test-real-merkle-proof-realization"), EXPECTED_HEALTH.index("test-generic-payload-full-enforcement")); self.assertLess(EXPECTED_HEALTH.index("test-generic-payload-full-enforcement"), EXPECTED_HEALTH.index("test-schemas"))
