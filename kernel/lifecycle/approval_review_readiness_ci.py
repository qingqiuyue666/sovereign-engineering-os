"""Flat read-only CI consumer for approval/review contract checks.

This module accepts only an already-rendered approval/review contract
payload and projects it to a bounded JSON-safe CI verdict. It does not
call contract checkers, envelope builders, data stores, operator
commands, or approval/review mutation paths.
"""
...
_SURFACE = "approval_review_readiness_ci"
_VERSION = 1
_INPUT_SHAPE = "rendered_approval_review_readiness_contract_check"
...
def approval_review_readiness_ci_manifest() -> dict[str, object]:
    """Return a defensive JSON-safe manifest for the CI consumer."""

    return deepcopy(_MANIFEST)


def consume_approval_review_readiness_ci(
    payload: object,
) -> dict[str, object]:
