...
``real_fix_verified_fail`` audit record emitted by ``RealFixTracer.run``
via the shared ``_emit`` helper when the extracted fenced Python block
executes but returns the wrong semantic result for at least one
verification case.
...
``_emit`` helper forwards the id so the ``real_fix_verified_fail``
...
prior audit shape exactly on the verified-fail record.

The ``real_fix_attempt_started`` / ``real_fix_adapter_failure`` /
``real_fix_unparseable`` / ``real_fix_exec_error`` /
``real_fix_verified_pass`` siblings landed in prior increments and are
not re-asserted here. Driving this through the live tracer is direct:
feed a fake transport whose fenced Python block defines a callable
``add`` implementation that returns the wrong answer so verification
fails and the tracer emits ``real_fix_verified_fail``.
"""
...
class TestRealFixVerifiedFailNamesIntentId(unittest.TestCase):
    """AUDIT-003 / §22.1 parity for ``real_fix_verified_fail``."""
    ...
    def setUp(self) -> None:
        ...
        self.audit = AppendOnlyLedger(
            ...
            actor_identity="test_real_fix_verified_fail_intent_id",
        )
    ...
    def _fetch_verified_fail_row(self, task_id: str) -> Any:
        return self.conn.execute(
            "SELECT payload_json, artifact_refs FROM audit_records "
            "WHERE task_id = ? "
            "AND record_type = 'real_fix_verified_fail' "
            "ORDER BY sequence DESC LIMIT 1;",
            (task_id,),
        ).fetchone()

    def test_verified_fail_names_intent_id_when_threaded(self) -> None:
        """A ``run`` call carrying ``intent_id`` whose extracted source
        executes but fails verification must emit a
        ``real_fix_verified_fail`` audit row whose ``payload`` and
        ``artifact_refs`` both name the threaded ``intent_id`` while
        preserving the prior ``case_index`` / ``args`` / ``expected`` /
        ``actual`` / ``output_hash`` / ``latency_ms`` payload fields
        and the prior ``[real-fix::<task_id>]`` refs contribution."""
        ...
        row = self._fetch_verified_fail_row(task.task_id)
    ...
    def test_absent_intent_id_preserves_prior_audit_shape(self) -> None:
        ...
        executes but fails verification must emit a
        ``real_fix_verified_fail`` audit row with the prior shape
