# Relocated Narrow Adapter Marker

This is the relocated non-runtime marker area for narrow adapter documentation markers.

It is intentionally outside kernel/adapters/.

kernel/adapters/ remains the pre-existing phase-1 real-model ignition adapter lane.

This marker area does not create adapter readiness, runtime readiness, execution readiness, or tool-control readiness.

This marker area does not authorize browser, shell, filesystem-wide mutation, DaVinci, Blender, Houdini, Unreal, ComfyUI, Photoshop, After Effects, or any local application control.

The only approved marker files in this package are:
...
This is a Markdown marker file only.
