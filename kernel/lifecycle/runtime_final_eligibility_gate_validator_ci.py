"""Read-only CI consumer for rendered runtime final eligibility validation."""
...
_SURFACE = "runtime_final_eligibility_gate_validator_ci"
_VERSION = 1
_CONTRACT_NAME = "RuntimeFinalEligibilityGateV1"
_VALIDATOR_PACKAGE = "runtime-final-eligibility-gate-validator-v1"
_VALIDATOR_CHECKPOINT_TAG = "runtime-final-eligibility-gate-validator-v1"
_VALIDATOR_CHECKPOINT_COMMIT = (
    "6a477db3e74bfba12a5041393ca37478149a0f3c"
)

_READY = "ready"
_NOT_READY = "not_ready"
_INVALID_CI_PAYLOAD = "invalid_ci_payload"
_INVALID_GATE_PAYLOAD = "invalid_runtime_final_eligibility_payload"
...
_SOURCE_REFS = {
    ...
    "runtime-final-eligibility-gate-spec-only-v1": (
        "1fc19a32b794bc36a00533ef2dacef473bff4672"
    ),
}

_COMPLETED_READ_ONLY_STACKS = tuple(
    name
    for name in _SOURCE_REFS
    if name != "runtime-final-eligibility-gate-spec-only-v1"
)
...
_REQUIRED_TRUE_DECLARATIONS = (
    ...
    "runtime_final_eligibility_forbidden",
    "runtime_final_gate_is_not_runtime",
...
_FUTURE_VALIDATOR_REQUIREMENTS = (
    "consumes_already_rendered_runtime_final_eligibility_gate_declarations_only",
    "read_only",
    "validates_exact_source_refs",
    "validates_exact_false_authority_flags",
    "validates_exact_true_declarations",
    "validates_exact_required_runtime_boundaries",
    "keeps_all_authority_false",
    "authorizes_no_runtime_or_service_call",
)

_FUTURE_CI_REQUIREMENTS = (
    "consumes_already_rendered_validator_output_only",
    "read_only",
    "binds_exact_validator_checkpoint_later",
    "keeps_all_authority_false",
    "authorizes_no_runtime_or_service_call",
)
...
_VALIDATOR_REASON_CODES = (_READY, _NOT_READY, _INVALID_GATE_PAYLOAD)
...
_EXPECTED_GATE_SUMMARY = {
    "surface": _CONTRACT_NAME,
    "version": _VERSION,
    "source_refs": deepcopy(_SOURCE_REFS),
    "completed_read_only_stacks": {
        stack: True for stack in _COMPLETED_READ_ONLY_STACKS
    },
    "required_runtime_boundaries": {
        boundary: True for boundary in _REQUIRED_RUNTIME_BOUNDARIES
    },
    "false_authority_flags": deepcopy(_AUTHORITY_SUMMARY),
    "true_declarations": {
...
_EXPECTED_VALIDATOR_GATE = {
    **deepcopy(_EXPECTED_GATE_SUMMARY),
    "json_safety": {
...
_CI_NON_AUTHORITY_SUMMARY = {
    "ci_ok_proves": (
        "already_rendered_runtime_final_eligibility_gate_validator_output_"
...
_FORBIDDEN_SURFACES = (
    ...
    "runtime final eligibility runtime",
    "runtime final gate runtime",
...
_MANIFEST = {
    ...
    "expected_gate_summary_keys": list(_GATE_SUMMARY_KEYS),
    "expected_validator_gate_keys": list(_VALIDATOR_GATE_KEYS),
    "expected_gate_summary": deepcopy(_EXPECTED_GATE_SUMMARY),
    "expected_validator_gate": deepcopy(_EXPECTED_VALIDATOR_GATE),
    "expected_source_refs": deepcopy(_SOURCE_REFS),
    "expected_completed_read_only_stacks": deepcopy(
        _EXPECTED_GATE_SUMMARY["completed_read_only_stacks"]
    ),
...
def runtime_final_eligibility_gate_validator_ci_manifest():
    """Return bounded metadata for the read-only validator CI consumer."""

    return deepcopy(_MANIFEST)


def consume_runtime_final_eligibility_gate_validator_ci(payload):
    ...
    if payload.get("runtime_final_eligibility_gate_ready") is True:
...
def _validate_readiness_reason_failures(payload, failures):
    ready = payload.get("runtime_final_eligibility_gate_ready")
    reason = payload.get("reason_code")
    source_failures = payload.get("failures")
    validator_failures = ()

    ready_valid = type(ready) is bool
    if not ready_valid and "runtime_final_eligibility_gate_ready" in payload:
    ...
    if ready_valid and reason_valid and failure_list_valid:
        if ready is True:
            ...
            if reason not in (_NOT_READY, _INVALID_GATE_PAYLOAD):
                _append_failure(failures, "reason_code_invalid")
            if source_failures == []:
                _append_failure(failures, "readiness_invalid")
...
def _validate_gate_summary(candidate, failures):
    ...
    if keys == set(_VALIDATOR_GATE_KEYS):
        if candidate != _EXPECTED_VALIDATOR_GATE:
            _append_failure(failures, "gate_summary_invalid")
        _validate_gate_authority_and_declarations(candidate, failures)
        return

    if keys == set(_GATE_SUMMARY_KEYS):
        if candidate != _EXPECTED_GATE_SUMMARY:
            _append_failure(failures, "gate_summary_invalid")
        _validate_gate_authority_and_declarations(candidate, failures)
        return
...
def _validate_gate_authority_and_declarations(candidate, failures):
    version = candidate.get("version")
    if type(version) is not int or version != _VERSION:
        _append_failure(failures, "gate_summary_invalid")

    flags = candidate.get("false_authority_flags")
    ...
    declarations = candidate.get("true_declarations")
...
def _validate_validator_non_authority_summary(candidate, failures):
    ...
    if not _is_json_safe(candidate, 0):
        ...
        _append_failure(failures, "json_safe_invalid")
        return
    if candidate != _VALIDATOR_NON_AUTHORITY_SUMMARY:
        _append_failure(failures, "non_authority_summary_invalid")
...
def _result(ci_ok, reason_code, failures):
    result = {
        ...
        "gate": deepcopy(_EXPECTED_GATE_SUMMARY),
