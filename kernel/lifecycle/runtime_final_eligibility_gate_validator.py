"""Read-only validator for RuntimeFinalEligibilityGateV1 declarations."""
...
_SURFACE = "runtime_final_eligibility_gate_validator"
_CONTRACT_NAME = "RuntimeFinalEligibilityGateV1"
_VERSION = 1

_READY = "ready"
_NOT_READY = "not_ready"
_INVALID_PAYLOAD = "invalid_runtime_final_eligibility_payload"

_INPUT_KEY = "runtime_final_eligibility_gate"
...
_MANIFEST = {
    ...
    "expected_completed_read_only_stacks": {
...
def runtime_final_eligibility_gate_validator_manifest():
    """Return bounded metadata for the read-only gate validator."""

    return deepcopy(_MANIFEST)


def validate_runtime_final_eligibility_gate(payload):
    """Validate an already-rendered RuntimeFinalEligibilityGateV1 mapping."""
...
def _validate_gate(gate, failures):
    ...
    _validate_true_mapping(
        gate.get("completed_read_only_stacks"),
        _COMPLETED_READ_ONLY_STACKS,
        "completed_read_only_stack_invalid",
        "completed_read_only_stack_invalid",
        failures,
    )
    _validate_runtime_boundaries(
        gate.get("required_runtime_boundaries"),
        failures,
    )
    _validate_false_authority_flags(
        gate.get("false_authority_flags"),
        failures,
    )
    _validate_true_declarations(gate.get("true_declarations"), failures)
    _validate_true_mapping(
        gate.get("json_safety"),
        _JSON_SAFETY_REQUIREMENTS,
        "json_safety_invalid",
        "json_safety_invalid",
        failures,
    ...
    _validate_true_mapping(
        gate.get("future_validator_requirements"),
        _FUTURE_VALIDATOR_REQUIREMENTS,
        "future_validator_requirement_invalid",
        "future_validator_requirement_invalid",
        failures,
    ...
    _validate_true_mapping(
        gate.get("future_ci_requirements"),
        _FUTURE_CI_REQUIREMENTS,
        "future_ci_requirement_invalid",
        "future_ci_requirement_invalid",
        failures,
...
def _result(failures):
    ...
    return {
        "runtime_final_eligibility_gate_ready": ready,
