...
VALIDATORS = {
    ...
    "validate_provider_transport_preflight": validate_provider_transport_preflight,
    "validate_provider_transport_receipt": validate_provider_transport_receipt,
    "validate_provider_transport_blocked_attempt": validate_provider_transport_blocked_attempt,
    "validate_provider_transport_postcheck": validate_provider_transport_postcheck,
}
...
class GatedProviderTransportContractsTests(unittest.TestCase):
    def load_gate(self):
        self.assertTrue(GATE_PATH.is_file(), str(GATE_PATH))
    ...
    def test_gate_map_is_accepted_contract_only(self):
        result = validate_provider_transport_gate(self.load_gate())
    ...
    def test_preflight_requires_manual_approval(self):
        payload = copy.deepcopy(self.valid_by_validator()["validate_provider_transport_preflight"])
        payload["manual_approval_present"] = False
        result = validate_provider_transport_preflight(payload)
        self.assertFalse(result.accepted)
        self.assertIn("manual_approval_missing", result.failures)

    def test_receipt_rejects_transport_attempt(self):
        payload = copy.deepcopy(self.valid_by_validator()["validate_provider_transport_receipt"])
        payload["transport_attempted"] = True
        result = validate_provider_transport_receipt(payload)
        self.assertFalse(result.accepted)
        self.assertIn("transport_attempted_must_be_false", result.failures)

    def test_postcheck_requires_all_links(self):
        payload = copy.deepcopy(self.valid_by_validator()["validate_provider_transport_postcheck"])
        payload["post_run_health_present"] = False
        result = validate_provider_transport_postcheck(payload)
        self.assertFalse(result.accepted)
        self.assertIn("post_run_health_present_required", result.failures)

    def test_non_mapping_payload_raises(self):
        with self.assertRaises(GatedProviderTransportContractViolation):
            validate_provider_transport_gate(["not", "mapping"])

    def test_makefile_declares_gated_provider_transport_gate(self):
        text = Path("Makefile").read_text(encoding="utf-8")
        self.assertIn("test-gated-provider-transport", text)
        self.assertIn("PYTHONDONTWRITEBYTECODE=1 $(PYTHON) -m unittest tests.tracer_bullet.test_gated_provider_transport_contracts -v", text)
    ...
    def test_runbook_exists_and_records_disabled_boundary(self):
        path = Path("docs/runbooks/gated_provider_transport_v1.md")
