...
class _LiveEvidenceView:
    ...
    def required_artifact_ids(
        ...
        # records and approval invalidation audit records, follow the same
        ...
        ids.extend(self._approval_invalidated_audit_ids(task_id))
        return ids
    ...
    def _approval_invalidated_audit_ids(self, task_id: str) -> list[str]:
        if self._audit_repo is None:
            return []
        ids: list[str] = []
        for record in self._audit_repo.list_approval_invalidated_for_task(
            task_id
        ):
            audit_record_id = record.get("audit_record_id")
            if isinstance(audit_record_id, str) and audit_record_id:
